    ...
    if last_transaction is None:
        ...
        if first_trade['type'] == 'buy':
            await database["transactions"].insert_one(
                {'pool_id': token['pool_id'], 'last_transaction': json.dumps(first_trade)})
            ...
            telegram_bot.send_message(
                message_text=telegram_message_format.format(format_count(count), token['base_token_name'],
                                                            ...
                                                            token['token_telegram'], token['description'], eth_value,
                                                            round(float(first_trade['totalUsd']), 2),
                                                            display_from_address, from_address, first_trade['txn'],
                ...
                if new_trade['type'] == 'buy':
                    ...
                    telegram_bot.send_message(
                        message_text=telegram_message_format.format(format_count(count), token['base_token_name'],
                                                                    ...
                                                                    eth_value, round(float(new_trade['totalUsd']), 2),
                                                                    display_from_address, from_address,
                                                                    new_trade['txn'], '{:,}'.format(current_market_cap),
